...
def to_mod_list(employee_list):
    ...
    employeeMap = map(mod,employee_list)
    stringList = []
    for employee in employeeMap:
        stringList.append(employee)
...
def map_id_to_initial(employee_list):
    ...
    for employee in employee_list:
        list1.append(employee["id"])
        list2.append(employee["name"][0])
    newDict = {key:value for (key,value) in zip(list2,list1)}
    return newDict
    raise NotImplementedError()
